...
module.exports.findOrCreateByValue = async function(options) {
  try {
    const base = new Airtable({ apiKey: process.env.AIRTABLE_API_TOKEN }).base(options.baseId);

    // Try to find the record by the specified field and value
    let foundRecords = [];
    await base(options.table).select({
      maxRecords: 1,
      view: options.view || "Grid view",
    ...
    cyan(`findOrCreateByValue - found records`, foundRecords);

    let record = foundRecords[0] || null; // Return the first record found or null if none
    ...
    if (record) {
      ...
      const newRecord = await base(options.table).create({
        [options.field]: options.value,
        ...options.additionalFields // Add any additional fields if necessary
      });

      cyan(`findOrCreateByValue - created new record`, newRecord);
      return newRecord;
    }
  } catch (error) {
    red("error in findOrCreateByValue", error);
    throw error; // Rethrow the error after logging
  }
}